...
function updateStatus(status, id) {
  return db
    .query(`UPDATE donations SET item_status=$1 WHERE id=$2`, [status, id])
    .catch((error) => {
      console.log(error);
    });
}
...
function countDonations(email) {
  return db
    .query(`SELECT COUNT(item_title) as count FROM donations WHERE email=$1`, [
      email,
    ])
    .then((result) => {
